      ...
      <div className='block'>
        <div className="ProductList mt-10   gap-1 sm:gap-4 md:gap-6 lg:gap-10 grid grid-cols-2 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 ">
          ...
          <NavLink key={index} to={`/AllProducts/${item.id}`} className='block border hover:border-[#bbb] rounded-none sm:rounded-xl'>
            <div style={ item.discount == null ?  DesibleDiscount : ShowDiscount} className="Discount absolute bg-green-700 text-white w-[80px] sm:w-[100px] text-center z-20 py-1 mt-10 ml-[0px] sm:ml-[-10px] rounded-sm ">
                <span className='text-sm sm:text-lg font-medium'>{item.discount}</span>% <span className='text-[10px] sm:text-sm'>Save</span>
            </div>
            <div className="Card  duration-300 overflow-hidden pb-6">
              ...
              <div className='line-ellipsetext text-[10px] sm:text-sm mt-4 sm:mt-6 px-5 sm:px-8'>Seller - {item.author}</div>
              <b className='p-ellipsetext mt-1 sm:mt-3 block px-5 sm:px-8'>{item.name}</b>
              ...
              <div className="Order flex items-center justify-between mt-3 px-5 sm:px-8">
                <h4>{item.price} <span className='text-lg'>$</span> <del className='StartingPrice text-sm ml-2 text-[#777]'>{item.startingprice}</del></h4>
